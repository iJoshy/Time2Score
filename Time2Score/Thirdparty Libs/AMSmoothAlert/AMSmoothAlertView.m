...
- (UIView*) alertPopupView
{
    ...
    /***************************************************************
     * alert frame determines area of alert
     ***************************************************************/
    
    
    UIView * alertSquare = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 270, 200)];
...
-(void) performScreenshotAndBlur
{
    ...
    [bg setImage:[image applyLightEffect]];
    bg.alpha = 0.0;
...
- (void) circleSetupForAlertType:(AlertType) type andColor:(UIColor*) color
{
    ...
    switch (type) {
        ...
        case AlertFailure:
            ...
            break;
        case AlertInfo:
            [_logoView setImage:[UIImage imageNamed:@"mobAssist.png"]];
            break;
        case AlertCustom:
            [_logoView setImage:[UIImage imageNamed:@"mobAssist.png"]];
            break;
...
- (void) labelSetupWithTitle:(NSString*) title andText:(NSString*) text
{
    ...
    /******************************************************
     *   title & text label to determine their frames
     ******************************************************/
    ...
    _titleLabel.font = [UIFont fontWithName:@"NeoTechAlt" size:17.0f];
    _titleLabel.textAlignment = NSTextAlignmentCenter;
    
    [alertView addSubview:_titleLabel];
    
    
    _textLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 230, 100)];
    _textLabel.center = CGPointMake(alertView.frame.size.width/2, 80);
    _textLabel.text = text;
    _textLabel.font = [UIFont fontWithName:@"NeoTechAlt" size:14.0f];
...
- (void) buttonSetupForType:(AlertType)type withCancelButton:(BOOL) hasCancelButton andColor:(UIColor*) color
{
    ...
    if (type == 3)
    {
        ...
        if (duration > 0)
        {
            ...
            durationTimer = [NSTimer scheduledTimerWithTimeInterval:duration
                                                             target:self
                                                           selector:@selector(handleButtonTouched:)
                                                           userInfo:nil
                                                            repeats:NO];
        }
    ...
    else
    {
        ...
        /***********************************************************
         *  center  160 determins distance of buttons from up
         ***********************************************************/
         
        if (hasCancelButton)
        {
            //default button
            _defaultButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 104, 33)];
            _defaultButton.center = CGPointMake((alertView.frame.size.width/4)+3, 160);

            //cancel button
            _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 104, 33)];
            _cancelButton.center = CGPointMake((alertView.frame.size.width*3/4)-3, 160);
            _cancelButton.backgroundColor = [UIColor redColor];
            
            [_cancelButton setTitle:@"Cancel" forState:UIControlStateNormal];
            _cancelButton.titleLabel.textColor = [UIColor whiteColor];
            _cancelButton.titleLabel.font = [UIFont fontWithName:@"NeoTechAlt" size:17.0f];
        ...
        {
            _defaultButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 230, 35)];
        ...
        _defaultButton.titleLabel.font = [UIFont fontWithName:@"NeoTechAlt" size:17.0f];
